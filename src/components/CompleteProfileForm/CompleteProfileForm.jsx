...
const CompleteProfileForm = () => {
  ...
  const validateForm = () => {
    ...
    if (!validatePhoneNumber(formData.phoneNumber)) return "Invalid phone number format (e.g., +12025550123).";
    if (!formData.country) return "Country is required.";
    if (!formData.birthdate) return "Birthdate is required.";
    if (new Date(formData.birthdate) > new Date("2025-03-02")) return "Birthdate cannot be in the future.";
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setError(error.message.includes("phoneNumber") ? "Invalid phone number format." : "Error updating profile. Please try again.");
